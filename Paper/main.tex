...
\begin{proof}
  ...
  \item \ref{item:sim-cf} \\
    ...
    \note{We need to show that $\tup{s(c), s(b)} \in S.\lPO$ implies $\tup{c, b} \in G.\lPO$}.
  ...
  \item \ref{item:sim-rf} \\
    ...
    \begin{equation*}
      \begin{split}
        ...
        & \forall{\tup{w, r} \in [I];G.\lRF;[C]} ~.~ \tup{s(w), s(r)} \in S.\lRF \wedge
          \forall{w \in I}. \tup{w, e} \in G.\lRF \implies \tup{s(w), \hat{e}} \in S.\lRF
      \end{split}
    \end{equation*}

    Note that we used the fact that $s'_{|C \cup I} = s_{|C \cup I}$.
    Left conjunct follows immediately from $\simRb(S, G, \tup{C, I}, s, t)$.
    Right conjunct follows from the definition of forwarding event.
