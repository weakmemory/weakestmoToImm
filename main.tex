...
\begin{definition}
  ...
  \begin{itemize}
    ...
    \item $S.\lRF \cup S.\lCF = \emptyset$;

    \item $(S.\lHB;S.\lRF^{-1}) \cup S.\lCF = \emptyset$;
...
\begin{definition}
  ...
  \begin{enumerate}[label=\textbf{S.\arabic*},start=5]
    ...
    \item \label{item:sim-pc}
      $\forall{i \in \Tid} ~.~ \tup{\lPC(i), \front(C, i)} \in G.\lPO$
...
\begin{proof}
  ...
  Let $t' = t$. Obviously \ref{item:sim-dom-t}-\ref{item:sim-jf} holds,
  because neither $S$ neither $t$ was modified. 
  We need to show that \ref{item:sim-rf}-\ref{item:sim-vis-weak} holds.
  ...
  \item $e \in G.\lW$.
    ...
    \item \ref{item:sim-mo} \\
      We need to show that $\forall{w \in I} ~.~ s'(w) = s(w)$.
