...
\begin{definition}
  ...
  %%   $\forall{\tup{\hat{e_1}, \hat{e_2}} \in [s(C)];S.\lPO;[s(C)]} ~.~
  %%       \tup{s^{-1}(\hat{e_1}), s^{-1}(\hat{e_2})} \in G.\lPO$;
...
\begin{lemma}
  ...
  there exists $e$ s.t. $G \vdash \tup{C, I} \rightarrow \tup{C \uplus \{e\}, I}$
  and $\hat{e} \in S.\lE$ s.t. $S.\lLAB(\hat{e}) = G.\lLAB(e)$,
  $\tup{s(\front{C, \lTID(e)}), \hat{e}} \in S.\lPO_{imm}$
  then $\simRb(S, G, \tup{C \uplus \{e\}, I}, s[e \mapsto \hat{e}], t[\hat{e} \mapsto e])$ holds. 
\end{lemma}

\begin{proof}
  Let $C' = C \mplus \{e\}$, $s' = s[e \mapsto \hat{e}]$ and $t' = t[\hat{e} \mapsto e]$.
  ...
  \item \ref{item:sim-po-prfx} \\
    ...
    follows that $\hat{a} \in C$. Contradiction.
...
\begin{definition}
  ...
  \begin{enumerate}[label=\textbf{I.\arabic*}]
    ...
    \item \label{item:sim-jf}
      ...
      \exists{w' \in \lC \cup \lI} ~.~ \tup{\lM(w'), \hat{r}} \in \lES.\lJF
      $
    \item \label{item:sim-mo}
      $\forall{\tup{w, w'} \in [\lI];\lG.\lMO;[\lI]} ~.~ \tup{\lM(w), \lM(w')} \in \lES.\lMO$;
    \item \label{item:sim-vis}
      $\forall{e \in \lC} ~.~ \lM(e) \in \lVIS(\lES)$;
  \end{enumerate}
\end{definition}

\begin{lemma}
  Given $\lES$, $\lG$, $\tup{\lC, \lI}$, $\lM$
  if the relation $\simR(\lES, \lG, \tup{\lC, \lI}, \lM)$ holds and 
  there exists $\tup{\lC', \lI'}$
  s.t. $\lG \vdash \tup{\lC, \lI} \rightarrow \tup{\lC', \lI'}$
  then there exists $\lES'$ and $\lM'$ s.t. $\lG \vdash (\lM, \lES) \ESstepstar (\lM', \lES')$
  and $\simR(\lES', \lG, \tup{\lC', \lI'}, \lM')$ 
\end{lemma}
