...
Section SimRelCertStepProps. 
  ...
  (* Lemma simrel_cert_basic_step_cert_rf k k' e e' S S'  *)
  (*       (st st' st'' : thread_st (ES.cont_thread S k)) *)
  (*       (SRCC : simrel_cert prog S G sc TC f TC' h k st st'')  *)
  (*       (BSTEP_ : ESBasicStep.t_ (thread_lts (ES.cont_thread S k)) k k' st st' e e' S S')  *)
  (*       (CST_REACHABLE : (lbl_step (ES.cont_thread S k))＊ st' st'') *)
  (*       (RR : exists is_ex ord loc val, ⟪ LBL_LD : Slab S' e = Aload is_ex ord loc val ⟫) : *)
  (*   exists w,  *)
  (*     ⟪hCertDOMw : (h □₁ cert_dom G TC (ES.cont_thread S k) st) w⟫ /\ *)
  (*     ⟪CertRF : cert_rf G sc TC' (ES.cont_thread S k) (e2a S' w) (e2a S' e)⟫. *)
  (* Proof.  *)
  (*   cdes BSTEP_. *)
  ...
  (*   assert (wf_thread_state (ES.cont_thread S k) st) as WFST. *)
  (*   { by apply SRCC. } *)
  (*   assert (lbl = Aload is_ex ord loc val) as LBL_EQ. *)
  (*   { rewrite <- LBL_LD, LAB'. *)
  ...
  (*   erewrite basic_step_e2a_e with (e := e);  *)
  (*     eauto; try apply SRCC. *)
  ...
  Lemma simrel_cert_store_step k lbl S
    ...
    exists k' e e' S', 
      ⟪ BSTEP_ : ESBasicStep.t_ (thread_lts (ES.cont_thread S k)) k k' st st' e e' S S' ⟫ /\
      ⟪ CertLSTEP  : cert_store_step k e e' S S' ⟫.
  Proof. 
    desf.
    ...
    { unfold ESstep.ew_delta, sim_ews. 
      erewrite basic_step_e2a_e with (e := ES.next_act S); 
        eauto; try apply SRCC. }
    unfold ESstep.co_delta, sim_ws. 
    arewrite (
     ESstep.co_ws (ES.next_act S) S S' ≡₁ 
       SE S ∩₁ SW S ∩₁ (fun w => Sloc S w = loc_lab lbl) \₁ ES.cont_cf_dom S k
    ).
    { unfold ESstep.co_ws.
      arewrite (
        SE S ∩₁ SW S ∩₁ same_loc (Slab S') (ES.next_act S) \₁ (Scf S') (ES.next_act S) ≡
        )
      rewrite <- set_interK with (s := SE S).
      repeat apply set_inter_Propere; auto. 
      { unfold same_loc. unfold Events.loc.
  ...
  Lemma simrel_cert_sim_add_jf k lbl lbl' lbls S ew co
    ...
    exists k' w e e' S',
      ...
      ⟪ LBL  : lbl  = S'.(ES.lab) e ⟫ /\
      ⟪ LBL' : lbl' = option_map S'.(ES.lab) e' ⟫ /\
      ⟪ CAJF : sim_add_jf G sc TC TC' h (ES.cont_thread S k) st w e S S' ⟫ /\
      ⟪ EW' : S'.(ES.ew) ≡ ew ⟫ /\
      ⟪ CO' : S'.(ES.co) ≡ co ⟫.
  Proof.
    desf.
    ...
    { erewrite basic_step_e2a_e with (e := e);
      ...
      { econstructor; eauto. }
      eapply a2e_img; eauto. apply SRCC. }
    cdes BSTEP_. rewrite EVENT. apply JF'.
  Qed.
